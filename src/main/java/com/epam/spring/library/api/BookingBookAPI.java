...
public interface BookingBookAPI {
    ...
    Set<BookDTO> getBooksInBooking(
            @PathVariable @Parameter(description = "booking id") int id);

    @Operation(summary = "Add books to booking")
    @PostMapping("/{id}/books")
    @ResponseStatus(HttpStatus.CREATED)
    Set<BookDTO> addBookListToBooking(
            @PathVariable @Parameter(description = "booking id") int id,
            @RequestBody @NotEmpty Set<@Valid BookDTO> bookDTOs);

    @Operation(summary = "Update books in booking")
    @PutMapping("/{id}/books")
    @ResponseStatus(HttpStatus.OK)
    Set<BookDTO> updateBooksListInBooking(
            @PathVariable @Parameter(description = "booking id") int id,
            @RequestBody @NotEmpty Set<@Valid BookDTO> bookDTOs);
